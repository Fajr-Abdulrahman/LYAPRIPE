...
emu_list_to_alphaAn_lya = np.zeros(len(k_z_combination_list), dtype= object)
emupars_list_to_alphaAn_lya = np.zeros(len(k_z_combination_list), dtype=object)
...
for i in range(len(k_z_combination_list)):
    outpars = np.array([ allparams_extras["alpha_lya"][:nsamples,i], allparams_essentials["A(k_pivot,z_pivot)"][:nsamples,i], allparams_essentials["n(k_pivot,z_pivot)"][:nsamples,i]])
    ...
    emu_list_to_alphaAn_lya[i]=emu 
    emupars_list_to_alphaAn_lya[i] = emupars
    k_piv_list[i] = k_z_combination_list[i,0]
    units_list[i] = k_z_combination_list[i ,1]
    z_piv_list[i] = k_z_combination_list[i,2]
np.savez_compressed("from_primordial_to_lya.npz", emu_to_alphaAn_lya=emu_list_to_alphaAn_lya, emupars_to_alphaAn_lya = emupars_list_to_alphaAn_lya, parnames=emuparnames, k_piv = k_piv_list, units = units_list, z_piv = z_piv_list, allow_pickle=True)
...
for i in range(len(k_z_combination_list)):
    inpars = np.array([ allparams_essentials["H0"][:nsamples], allparams_extras["alpha_lya"][:nsamples,i], allparams_essentials["A(k_pivot,z_pivot)"][:nsamples,i], allparams_essentials["n(k_pivot,z_pivot)"][:nsamples,i]])
    smooth_lengths = np.array(5*np.std(inpars, axis=1))
    emu, update_emu, emupars, emuparnames = create_emulator(  
        inpars.T,
        outpars.T,
